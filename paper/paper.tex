...
We fit the play-level model on all eight weeks of data (8{,}531 plays, 36{,}259 rusher observations) with the No-U-Turn sampler \citep{hoffman2014no} implemented using NumPyro \citep{phan2019composable}, running four chains of $1{,}000$ warmup and $2{,}000$ retained iterations each ($8{,}000$ posterior draws). Convergence diagnostics for this and every other sampled model are reported in Appendix~\ref{sec:diagnostics}.  We rank rushers, restricting our attention to rushers with at least 50 pass snaps over the sample.  This removes the rare blitzers with high variance estimates (e.g., defensive backs who rush on a handful of plays).
...
$$ \Delta\text{STRAIN}_{ijt} \sim \mathcal{N}\Big( \mu - \!\!\sum_{b \in \mathcal{X}_i}\!\! B_{b}\,\theta(b, j, t, i),\; \sigma_{\Delta}^2 \Big). $$
 $B_b$ is fit with the same attribute-centred prior as the play-level model, with its own scale $\sigma_{B}^{\Delta}$ and residual $\sigma_{\Delta}$ distinct from the play-level $\sigma_B$ and $\sigma_y$; $B_b > 0$ now represents a blocker whose engagement decelerates the rush. 

The continuous-time  model gives a blocker rating derived from STRAIN dynamics rather than play-level aggregates. Fit by NUTS on the $1.1$M rusher-frames over eight weeks, again with four chains ($800$ warmup and $800$ retained iterations each, $3{,}200$ draws), the per-blocker effect $B_b$ correlates most strongly out of our blocker ratings against independent PFF charting on the common set of $207$ linemen (Table~\ref{tab:blocker_cont_vs_play}). It leads on both pressures allowed (Pearson $-0.35$) and the instantaneous beaten event ($-0.41$). Crucially it also separates protection skill within position (Pearson $-0.25$). The leaderboard (Figure~\ref{fig:blocker_continuous}) is led by interior linemen who concede little charted pressure --- Rodney Hudson, Quenton Nelson, Connor McGovern, Wyatt Teller.
...
\caption{Posterior distributions of the continuous-time blocker rating $B_b$ for the top and bottom twelve pass blockers ($\geq 100$ snaps), coloured by position. Vertical lines mark the $2.5$, $50$ and $97.5$ percentiles. Zero is the average player at the position. Like the play-level coefficient, this is an effect per unit of assignment.}
...
where $\mathcal{E}_i = diag(E_{t_ib_ij_i}^{(s)})$ over the rows of $X_i$. Because the emission is isotropic, both field coordinates are pooled into this single variance: each blocker--frame contributes two residuals, so the denominator is twice the total responsibility mass.
...
Three of the models in this paper are fit by the No-U-Turn sampler: the play-level plus-minus model of Section~\ref{plusminus}, the continuous-time blocker rating of Section~\ref{continuous}, and the opponent-adjusted block-hold hazard of Section~\ref{shedding}. Each was run as four chains from dispersed initializations --- $1{,}000$ warmup and $2{,}000$ retained iterations per chain for the play-level model, $800$ and $800$ for the continuous-time model, and $1{,}000$ and $1{,}000$ for the block-hold model. Running more than one chain is what makes the rank-normalized $\widehat{R}$ of \citet{vehtari2021rank} informative: it compares the between-chain and within-chain variance of each parameter, and so detects chains that have settled in different regions of the posterior, which no single-chain diagnostic can.

Table~\ref{tab:mcmc_diagnostics} reports $\widehat{R}$ and the bulk and tail effective sample sizes for every parameter the paper discusses. Diagnostics are computed on the quantities we actually report rather than on the underlying standardized deviations, so the per-player entries are the transformed effects $R_j$, $B_b$, $Q_q$ and $-u_b$; for these vector-valued entries the table gives the worst value over all components.

The play-level and block-hold models mix well: every parameter has $\widehat{R} \leq 1.006$ with bulk and tail effective sample sizes in the hundreds to thousands. The continuous-time model is the exception. Its blocker attribute weights $\alpha_B$ ($\widehat{R} = 1.027$, bulk ESS $155$) and per-blocker effects $B_b$ ($\widehat{R} = 1.016$, bulk ESS $184$) fall short of the conventional $\widehat{R} \leq 1.01$ and ESS $\geq 400$ thresholds. This is visible in the sampler's behaviour as well as its output: one of the four chains consistently required five to eight times as many leapfrog steps per iteration as the other three, indicating it explored a region of the posterior the others did not reach. The point estimates are nonetheless stable --- the leaderboard and the correlations against PFF charting reproduce those of independent single-chain fits to within $0.01$ --- but the credible intervals reported for $B_b$ should be read as less securely estimated than those of the play-level model. We report this rather than tuning it away because the continuous-time likelihood, with $1.1$M rusher-frames informing $534$ blocker effects through a single shared scale, is exactly the geometry in which such behaviour is expected.

We additionally monitored divergent transitions. The play-level model produced $61$ divergences out of $8{,}000$ post-warmup draws ($0.8\%$), concentrated in the tails of the variance parameters; the other two models produced none. A divergence rate at this level does not invalidate the fit, but it is a further reason to treat the smallest variance components as approximate.

\input{../tables/mcmc_diagnostics.tex}
...
\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{../figures/ridge_blocker_pm_all.pdf}
\caption{Posterior distributions of the play-level plus-minus blocker effect $B_b$ for the top and bottom five pass blockers by position (min 50 pass snaps). Higher values indicate better pressure suppression. Zero is the average player at the position.}
...
Because the quarterback intercept $Q_{q_i}$ also enters the peak-STRAIN predictor additively, the model yields a by-product ranking of quarterbacks by how much pressure their dropbacks tend to attract. Reporting this as a suppression score, Figure~\ref{fig:qb_suppression} lists the most and least STRAIN-suppressing passers. Those who suppress the most STRAIN (Kyler Murray, Matthew Stafford, and Jared Goff) are known for mobility and a quick release, whereas those who allow the most (Zach Wilson, Davis Mills, and Jameis Winston) were among the most-pressured quarterbacks of the 2021 season. We note that this score reflects STRAIN rather than sacks, so a scrambler such as Lamar Jackson can rate as allowing pressure even while avoiding sacks. The posterior distributions are wide, and overlap, indicating that there might not be enough signal in the data to identify these effects.  

\begin{figure}[h!]
\centering
\includegraphics[width=0.72\textwidth]{../figures/ridge_qb_suppression.pdf}
\caption{Posterior distributions of quarterback STRAIN suppression $-Q_q$ for the ten most and ten least suppressing passers (min 50 dropbacks). Higher values indicate less peak pressure on that quarterback's dropbacks.}
