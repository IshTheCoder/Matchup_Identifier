...
We fit the play-level model on all eight weeks of data (8{,}530 plays, 36{,}254 rusher observations) with the No-U-Turn sampler \citep{hoffman2014no} implemented using NumPyro \citep{phan2019composable}, running four chains of $1{,}000$ warmup and $2{,}000$ retained iterations each ($8{,}000$ posterior draws). Convergence diagnostics for this and every other sampled model are reported in Appendix~\ref{sec:diagnostics}.  We rank rushers, restricting our attention to rushers with at least 50 pass snaps over the sample.  This removes the rare blitzers with high variance estimates (e.g., defensive backs who rush on a handful of plays).
...
We extend the play-level plus-minus model to continuous time by resolving it to the (play $i$, rusher $j$, frame $t$) level, so that every rusher-frame is a distinct observation rather than a single play-level summary. We take the one-frame change in STRAIN as the response, $\Delta\text{STRAIN}_{ijt} = \text{STRAIN}_{i,j,t+1} - \text{STRAIN}_{ijt}$. The regressor is the blocking effect a rusher receives at $t$, weighted by $\theta(b,j,t)$ much like the play-level model. The effect carries its own symbol $B^{\Delta}_b$, distinct from the play-level $B_b$ because the two are different parameters on different scales: $B^{\Delta}_b$ reflects how much blocker $b$'s engagement decelerates the rusher over the next frame.
$$ \Delta\text{STRAIN}_{ijt} \sim \mathcal{N}\Big( \mu - \!\!\sum_{b \in \mathcal{X}_i}\!\! B^{\Delta}_{b}\,\theta(b, j, t, i),\; \sigma_{\Delta}^2 \Big). $$
 $B^{\Delta}_b$ is fit with the same attribute-centred prior as the play-level model, with its own scale $\sigma_{B}^{\Delta}$ and residual $\sigma_{\Delta}$ distinct from the play-level $\sigma_B$ and $\sigma_y$; $B^{\Delta}_b > 0$ represents a blocker whose engagement decelerates the rush. 

The continuous-time  model gives a blocker rating derived from STRAIN dynamics rather than play-level aggregates. Fit by NUTS on the $1.1$M rusher-frames over eight weeks, again with four chains ($800$ warmup and $800$ retained iterations each, $3{,}200$ draws), the per-blocker effect $B^{\Delta}_b$ correlates most strongly out of our blocker ratings against independent PFF charting on the common set of $207$ linemen (Table~\ref{tab:blocker_cont_vs_play}). It leads on both pressures allowed (Pearson $-0.35$) and the instantaneous beaten event ($-0.40$), and it also separates protection skill within position (Pearson $-0.25$), though on that last comparison the front-normalized impedance of Eq.~\ref{eq:fn} is the stronger discriminator ($-0.43$). The leaderboard (Figure~\ref{fig:blocker_continuous}) is dominated by interior linemen who concede little charted pressure --- Matt Skura, Ereck Flowers, Jon Runyan, Connor McGovern and Quenton Nelson all appear --- alongside a handful of tackles who rate well once each blocker is measured against his own position.
